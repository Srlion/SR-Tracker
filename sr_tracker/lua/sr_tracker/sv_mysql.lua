...
mysql.Module = "sqlite" -- sqlite, tmysql4, mysqloo
mysql.Host = "127.0.0.1" -- Use 127.0.0.1 for localhost
mysql.Username = ""
mysql.Password = ""
mysql.Database = ""
mysql.Port = 3306
...
function QUERY_CLASS:Escape(text)
	return sr_mysql:Escape(tostring(text));
end;
...
local function BuildDropQuery(queryObj)
	local queryString = {"DROP TABLE IF EXISTS"}
...
function QUERY_CLASS:Execute(bQueueQuery)
	...
	if (type(queryString) == "string") then
		if (!bQueueQuery) then
			return sr_mysql:RawQuery(queryString, self.callback);
		else
			return sr_mysql:Queue(queryString, self.callback);
		end;
	end;
end;
...
function sr_mysql:RawQuery(query, callback, flags, ...)
...
function sr_mysql:Queue(queryString, callback)
	if (type(queryString) == "string") then
...
function sr_mysql:SetModule(moduleName)
	Module = moduleName;
end;

-- Called when the database connects sucessfully.
function sr_mysql:OnConnected()

   	Connected = true;
	hook.Run("SR_Tracker.DatabaseConnected");
end;

-- Called when the database connection fails.
function sr_mysql:OnConnectionFailed(errorText)
	ErrorNoHalt("[mysql] Unable to connect to the database!\n"..errorText.."\n");

	hook.Run("SR_Tracker.DatabaseConnectionFailed", errorText);
end;

-- A function to check whether or not the module is connected to a database.
function sr_mysql:IsConnected()
	return Connected;
end;

--=================================================================================================================================================================================================--

function sr_mysql:FormatQuery(str, ...)
	sr_mysql:RawQuery(str:format(...))
end

--=================================================================================================================================================================================================--

hook.Add("Initialize", "SR_Tracker.Load", function()
	sr_mysql:Connect(mysql.Host, mysql.Username, mysql.Password, mysql.Database, mysql.Port)

	timer.Create("SR_Tracker.QueryQueues", 1, 0, function()
		sr_mysql:Think()
	end)
end)
